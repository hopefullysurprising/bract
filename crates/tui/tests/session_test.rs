...
// A task that is both runnable and a parent (`app:check`) must open its form on
// Enter and run with the full colon-joined task path — not `mise run app check`.
#[test]
fn runnable_branch_task_runs_with_joined_path() {
    let tools = build_task_tools();
    let mut session = Session::new(tools, 80, 40);
    session.select_command(&["Mise Tasks", "app", "check"]);
...
fn deep_leaf_task_runs_with_joined_path() {
    let tools = build_task_tools();
    let mut session = Session::new(tools, 80, 40);
    session.select_command(&["Mise Tasks", "app", "check", "be", "test", "cov"]);
    let spec = session.run();
